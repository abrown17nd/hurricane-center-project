...
# Make sure 'datetime' column is parsed as datetime if needed
if 'datetime' in best_track.columns:
    best_track['datetime'] = pd.to_datetime(best_track['datetime'])
...
def latlon_to_pixel(lat, lon):
    # Replace with actual conversion using DOMAIN_DATA
...
def update_image():
    ...
    if filename in label_dict:  # optionally, check some BB dict
        row = label_dict[filename]  # adjust this if BB info is in a separate structure
        draw.rectangle([row["x_min"], row["y_min"], row["x_max"], row["y_max"]], outline="red", width=2)

    # Draw best-track circle if matching storm found
    subset = best_track[best_track["storm_name"].str.contains(str(filename.split("_")[0]), na=False)]
    if "datetime" in best_track.columns:
        # optional: match by closest datetime if you have it
        subset = subset.head(1)  # simplest version
    if not subset.empty:
        bt_row = subset.iloc[0]
        x_bt, y_bt = latlon_to_pixel(bt_row["lat"], bt_row["lon"])
        r = 8
